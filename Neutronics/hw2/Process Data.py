...
E1, U238_ng = np.loadtxt('U238ng.txt', skiprows=1, unpack=True, delimiter=',')
E2, U238_es = np.loadtxt('U238es.txt', skiprows=17, unpack=True, delimiter=',')
E3, H1_es = np.loadtxt('H1es.txt', skiprows=1, unpack=True, delimiter=',')

Emesh = np.linspace(1, 50, 1200)
...
def compute_spectrum(E, Sigma_t, Sigma_s, Nk, alpha) :
    N_E = len(E)
    phi = np.zeros(N_E)
    phi[N_E-1] = 1.0
    for i in range(N_E-2, -1, -1) :
        ...
        for j in range(N_E-1, i, -1) :
            ...
            for k in range(len(Nk)):
                Q_i += phi[j] * (Sigma_s[k][j]*Nk[k]) * scatter_probability(E[i], E[j], alpha[k]) * dE
        phi[i] = Q_i / Sigma_t[i]
        #print (i, phi[i])
    return phi
...
plt.xlabel('Energy [eV]'), plt.ylabel('$\phi(E)$')


Nk = [100.0, 1.0]
Ak = [alpha(1), alpha(238)]
St = 1.0*(U238_tot)+100.0*H1_es
Ss = [H1_es, U238_es]

phi = compute_spectrum(Emesh, St, Ss, Nk, Ak)  
ax.loglog(Emesh, phi/phi[0]*max(1/Emesh*Emesh[-1]))
plt.legend(['$\phi_{nr}$','$\phi_{wr}$','1/E', 'Slowing Down $\phi$'])
plt.tight_layout()
