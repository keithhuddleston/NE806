...
def Group_Plot(Nuclide, Temperature, Type, group_structure):
    file_name = 'Data/Group/2/'+Nuclide.N+'G_'+Type+'_'+str(300)+'.txt'
...
def Temperature_Plot(Nuclide, Temperature, Type, group_structure):
    ...
    group = np.loadtxt(file_name, skiprows=1, unpack=True)  
    y_vals_group1 = group[-1]
    file_name = 'Data/Group/16/'+Nuclide.N+'G_'+Type+'_'+str(1200)+'.txt'
    group = np.loadtxt(file_name, skiprows=1, unpack=True)  
    y_vals_group2 = group[-1]
    ...
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    plt.xlabel('Energy (eV)'), ax1.set_ylabel('Cross Section (Barns)')
    ax1.plot(e, s, color='black')
    file_name = 'Data/Doppler/'+Nuclide.N+'_'+Type+'_1200'+'.txt'
    e, s = np.loadtxt(file_name, skiprows=1, unpack=True, delimiter=',')
    ax1.plot(e, s, color='blue')
    ax1.set_xlim(min(group_structure), max(group_structure)) 
    for k in range(len(group_structure)-1):
    ...
    line1 = Mlines.Line2D([], [], color='k', label=r'$\sigma_{t}(E), T=300\degree K$')
    line2 = Mlines.Line2D([], [], color='b', label=r'$\sigma_{t}(E), T=1200\degree K$')
    line3 = Mlines.Line2D([], [], color='r', label=r'$\sigma_{tg},\ T=300\degree K$')
    line4 = Mlines.Line2D([], [], color='g', label=r'$\sigma_{tg},\ T=1200\degree K$')
...
# Plotting data
# ============================================================================
Group_Plot(U235, 300, 'Total', Casmo_2)
